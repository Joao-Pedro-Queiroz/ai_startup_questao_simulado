...
public class TextCleaner {
    ...
    public static String cleanText(String text) {
        ...
        // Captura casos onde há LaTeX no meio: "x = \frac{3(y-z)}{5}x = 53(y-z)"
    ...
    private static String removeCompleteEquationDuplicates(String text) {
        ...
        while (matcher.find()) {
            ...
            // CORREÇÃO: Escape correto dos colchetes dentro da classe de caracteres
            String clean1 = norm1.replaceAll("[\\[\\](){}]", "").replaceAll("\\\\", "").replaceAll("[+\\-*/=]", "").replaceAll("frac", "")
                    .replaceAll("\\d+", "N"); // Normaliza números para "N"
            String clean2 = norm2.replaceAll("[\\[\\](){}]", "").replaceAll("\\\\", "").replaceAll("[+\\-*/=]", "").replaceAll("frac", "")
                    .replaceAll("\\d+", "N"); // Normaliza números para "N"
            ...
            // CORREÇÃO: Usa Matcher.quoteReplacement para escapar caracteres especiais ($ e \)
    ...
    private static String removeDuplicateExpressions(String text) {
        ...
        while (matcher.find()) {
            ...
            // CORREÇÃO: Usa Matcher.quoteReplacement para escapar caracteres especiais
    ...
    private static String removeLatexTextDuplicates(String text) {
        // Procura por padrões LaTeX inline seguidos de texto similar
        ...
        while (matcher.find()) {
            ...
            // Extrai o conteúdo LaTeX do bloco removendo as tags
            String latexContent = latexBlock.replaceAll("^\\\\\\(", "").replaceAll("\\\\\\)$", "");
            // Remove comandos LaTeX comuns para comparação
            ...
            // Normaliza o texto após o LaTeX
            ...
            // Se o texto após o LaTeX parece ser uma duplicação do conteúdo LaTeX
    ...
    private static String removeTextLatexDuplicates(String text) {
        ...
        while (matcher.find()) {
            ...
            // Normaliza o texto antes do LaTeX
            ...
            if (textNorm.length() > 2 && latexNorm.length() > 2) {
                ...
                if (textNorm.substring(0, compareLen).equals(latexNorm.substring(0, compareLen))) {
                    // Retorna só o LaTeX (mais completo)
                    matcher.appendReplacement(sb, Matcher.quoteReplacement(latexBlock));
                    continue;
                }
            }
    ...
    /**
     * Análise de metade do texto - detecta duplicações grandes (última verificação)
     */
    private static String removeLargeDuplicates(String text) {
        ...
        if (firstHalf.length() > 8 && secondHalf.length() > 8) {
            // Remove tags LaTeX para comparação
            ...
            if (compareLen > 8 && firstClean.substring(0, compareLen).equals(secondClean.substring(0, compareLen))) {
                // Se a primeira metade tem mais LaTeX, mantém ela
                ...
                if (firstHasLatex && !secondHasLatex) {
                    ...
                    // A segunda tem LaTeX, mas está na segunda metade
                ...
                // Se forem similares, pega só a primeira metade
